...
2. Make the smallest complete fix at the level that owns the violated rule or contract. Preserve every reviewed abstraction, project module constraint, dependency direction, and reuse decision. Recheck the documented existing capability before adding helpers or adapters; extend its owner when the contract fits rather than copying behavior. Support files or adjusted forecast paths are acceptable when those constraints remain intact; explain meaningful placement differences in conformance. If a boundary or module constraint was missed, restart design review rather than coupling policy to concrete infrastructure.
