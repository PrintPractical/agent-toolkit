...
Before discovery, obtain the goal and observable outcome, affected area, constraints and anti-goals, and whether requirements are formed, partially formed, or unformed. Do not read references, scan the repository, discover context, or launch subagents first. After scope is established, read only the applicable shared references and use the `idioms` skill only for relevant languages.
...
## Phase 0: Intake, scope, and workspace

Obtain the intake, then use the supplied scope: a path, component, layer, package, set of targets, or whole stack. Normalize it to concrete repository-relative paths and record both requested and effective scope in `refactor.md`.
...
Write `.changes/active/<id>/change-brief.md` from `references/templates/change-brief.md.tmpl` and confirm `manifest.artifacts.change_brief` names it.
...
3. Detect languages that contribute to the selected work or its reviewable behavior, including relevant mixed-language packages and first-party build/configuration logic.
4. Use the `idioms` skill only for languages that contribute to the selected work or its reviewable behavior. Record loaded and missing applicable packs.
...
## Phase 3: Explicit selection approval
...
```bash
node "$SKILL_DIR/scripts/manifest-approval.mjs" --id <id> --approval refactor --approve
```
...
Preserve the artifact and report the blocker. Do not improvise around an approval.
...
- `idioms` skill - load applicable packs only
