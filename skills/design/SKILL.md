...
5. Have a separate context complete every checklist item, then make the cycle's one comprehensive discovery pass over requirements traceability, applicable project instructions, support envelope, examples, rules, boundaries, abstraction decisions, placement, risks, and vertical slices. It must catch omitted requirements and abstractions that lack a planned implementation, consumer, implementation, composition point, or test. Findings require concrete contract references, evidence, and observable impact. Zero findings is valid; do not optimize for findings or speculative hardening.
...
10. Record verifier approval, confirm `ready-to-build`, then end the design workflow. Do not run `agent-toolkit advance` from `ready-to-build`; the build skill owns the transition to `implementing`.
