...
5. Before editing, extract from reviewed contracts: dependencies, integration points, module constraints, placement, milestones, slices, and reuse decisions.

## Implement

1. Use `agent-toolkit advance` to enter `implementing` when permitted.
2. Follow the artifact's implementation plan in slice order. Implement only the next slice reported by `status`. Complete its entry point, core behavior, integration, and tests before starting another slice; do not implement phase-only batches.
3. Preserve reviewed rules, contracts, errors, dependencies, and project instructions. Before adding a helper or abstraction, recheck its documented existing capability; extend it when the contract fits instead of copying behavior. Parallel code requires the reviewed distinction.
4. Update the change artifact when implementation evidence changes a decision. Additional support files or path adjustments do not require design review when responsibilities and project module constraints remain intact; explain meaningful non-material placement deviations in `Implementation Conformance`. For material product, integration, public-interface, module-constraint, or plan changes, run `agent-toolkit review restart --stage design`; this returns the revision to developer feedback before fresh review.
...
Treat reviewed abstractions and project instructions as acceptance criteria. When implementation reveals a material missed constraint or integration decision, stop and restart design review instead of changing the design to justify a convenience-driven deviation.
...
4. Ask the critic to complete every item in the prepared packet's checklist before writing findings. Make the cycle's one comprehensive discovery pass over the sealed diff for correctness, requirements delivery, and reviewed-contract conformance. It must catch omitted requirements, reviewed abstractions or implementations left as scaffolding or unused, incomplete slices, regressions, and project constraints. Every finding needs a concrete contract reference, candidate evidence, and observable impact. Zero findings is valid; do not optimize for findings, speculative hardening, or behavior outside the applicable support envelope.
