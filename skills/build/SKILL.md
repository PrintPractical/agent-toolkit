...
4. Update the change artifact when implementation evidence changes a decision. Additional support files, exact filenames, or local implementation details do not require design review when reviewed ownership, roles, module constraints, dependencies, reuse decisions, abstractions, and slices remain intact; explain meaningful non-material path deviations in `Implementation Conformance`. If the decomposition is materially wrong or incomplete, or any of those reviewed constraints must change, run `agent-toolkit review restart --stage design` before implementing the deviation; this returns the revision to developer feedback before fresh review.
...
1. Reconcile code, tests, the change artifact, and system map. Complete `Implementation Conformance` by mapping every reviewed owner, role, dependency, reuse decision, abstraction, project module constraint, expected placement, and exact individual slice to code and verification evidence. Explain acceptable path differences; any material ownership or architecture deviation requires design review restart. Run missing relevant evidence only; reuse current executable-fingerprint evidence when the candidate is unchanged.
