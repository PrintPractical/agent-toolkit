...
4. Ask the critic to complete every item in the prepared packet's checklist before writing findings. Make one comprehensive pass over the sealed diff for correctness, requirements delivery, domain clarity, and exact architecture conformance. It must map reviewed ports, adapters, dependency directions, composition and transaction boundaries, project module constraints, and expected placement to the candidate, then inspect cohesion, test quality, regressions, and design drift. It must distinguish acceptable forecast-path differences from violated responsibilities or organization rules. Concrete infrastructure leaking into an inward layer, an omitted reviewed abstraction, or a violated `AGENTS.md` module constraint is a material finding. Do not stop after the first defect; report all findings now, not optional improvements later.
