...
5. Before editing, extract from reviewed contracts: ports, adapters, dependency direction, composition, transaction boundaries, module constraints, placement, milestones, slices, and reuse decision.
...
3. Preserve reviewed rules, boundaries, contracts, errors, and dependency direction. Implement ports and adapters before concrete infrastructure; compose at the edge. Before adding a helper, rule, or adapter, recheck its documented existing capability; extend its owner when the contract fits instead of copying behavior. Parallel code requires the reviewed distinction.
