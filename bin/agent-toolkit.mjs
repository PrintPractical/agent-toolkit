...
async function review() {
  ...
  if (action === "prepare") {
    ...
    const criticInstructions = state.type === "project"
      ...
      : packet.stage === "design"
        ? "Perform one bounded discovery pass over supported-now requirements, applicable project instructions, responsibility owners, reuse decisions, placement, planned slices, and demonstrated risks. Confirm deferred and non-goal behavior stays outside implementation and acceptance. Reject semantic duplication, governing boundary violations, purposeless abstractions, vague placement, and a production-code or source-file growth budget disproportionate to the supported outcome. Report blocker defects only. Zero findings is valid."
        : "Perform one bounded discovery pass over the sealed change, supported-now contract, and causally affected callers. Compare Implementation Conformance, including actual production-code growth, largest source-file impact, and added dependencies or abstractions, with the reviewed budget. Catch regressions, incomplete supported behavior, duplicated ownership, new cross-boundary dependencies, violated project constraints, unused scaffolding, and slices that do not build and run. Report blocker defects only. Zero findings is valid; do not request hypothetical, deferred, or optional cleanup work.";
    ...
    console.log(JSON.stringify({
      ...packet,
      instructions: `Review only the supplied project framing, change design when present, system map, canonical candidate, test evidence, and findings. Use one fresh critic context and one distinct verifier context per cycle; reuse that verifier context for closure retries. ${packet.role === "critic" ? criticInstructions : verifierInstructions} A packet finding is a workflow blocker, not advice: it must identify a supported scenario, concrete reviewed contract violation, candidate evidence, observable impact, and the smallest required correction. Preferences, speculative hardening, deferred or unsupported cases, and optional cleanup stay advisory and must not appear as findings or change an approval verdict. Write JSON matching the supplied schema directly to findingsPath, including {"findings":[]} for approval. Do not create review output anywhere else in the project.`,
      outputSchema: {
        type: "object",
        additionalProperties: false,
        required: ["findings"],
