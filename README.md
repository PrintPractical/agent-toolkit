...
A **change** is one coherent `feature` or `fix` candidate that can share one design contract, quality review, and inspected commit. Its contract classifies behavior as supported now, deferred, or a non-goal. Only supported-now behavior can drive implementation, acceptance, or blocker findings. Artifact depth is proportional to applicability; there is no formal small/medium classification. A one-slice correction and a multi-slice feature use the same mandatory gates.
...
Project framing is not predictive architecture. Concrete APIs, schemas, boundaries, and module placement emerge during milestone design by applying repository instructions, especially `AGENTS.md`. Every change records the smallest viable approach and a reviewed budget for production-code growth, affected files, largest source-file impact, and new dependencies or abstractions. It then decomposes significant supported behavior into authoritative responsibility owners, records a `REUSE`, `EXTEND`, `REFACTOR`, or `NEW` decision from repository evidence, and maps each owner to placement and dependencies. Implementation reconciles actual complexity against that budget before quality review; disproportionate growth triggers simplification or renewed design rather than silent code accumulation. One cohesive owner and one slice remain valid for a simple change.
...
Review packets use protocol 3 JSON only. Every response, including approval, is written to the packet's exact runtime-only `findingsPath` as `{"findings":[]}` or structured high/medium blocker findings with `contractReference`, `evidence`, and `observableImpact`. A finding must demonstrate a supported scenario and reviewed-contract violation; preferences, speculative hardening, deferred behavior, and optional cleanup remain advisory outside the packet and cannot block delivery. Critic and verifier identities must differ; closure retries reuse the verifier identity. A verifier can only reopen supplied findings, reject inaccurate dispositions, or report a demonstrated high-severity regression introduced by remediation.
