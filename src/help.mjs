...
Validates artifact presence, requirements traceability, responsibility decomposition, architecture placement and reuse decisions, vertical implementation slices through reviewed owners, the system map, and closed questions. During implementation it also validates Architecture Decisions and the active slice's Implementation Conformance before slice completion.`,
...
Records one reviewed slice as complete. Before running it, complete Architecture Decisions with Decision, Owners, Implementation, and Verification; Owners must contain every reviewed owner exactly once. Add the active slice's exact reviewed title plus Implementation and Verification under Slice Completion, without future slice records. The current candidate must also have passing acceptance evidence from the exact command reviewed in the plan.`,
  review: `Usage:
